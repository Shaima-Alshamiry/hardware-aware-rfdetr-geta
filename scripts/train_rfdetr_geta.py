...
# ==========================================
# 1. OPTIMIZER & GRAPH PATCHES (To fix OTO errors)
# ==========================================
import torch.optim.optimizer
import torch.nn.functional as F

# 1. Fix Optimizer Duplicate Parameters issue
...
# 2. Fix Bicubic issue (to prevent OTO/ONNX crash)
...
# 3. 🎯 [The New and Strongest Patch]: Fix Dynamic Shapes & ListConstruct issue
def ensure_static_int(s):
    # This function ensures that the number is a pure Int and not a Tensor to satisfy ONNX
...
def patched_view(self, *shape):
    # Unpack the Shape if it is inside a list or Tuple
    if len(shape) == 1 and isinstance(shape[0], (tuple, list)): 
        shape = shape[0]
    # Force each element to be a Static Integer
    static_shape = [ensure_static_int(s) for s in shape]
    return _original_view(self, *static_shape)
...
def patched_reshape(self, *shape):
    if len(shape) == 1 and isinstance(shape[0], (tuple, list)): 
        shape = shape[0]
    static_shape = [ensure_static_int(s) for s in shape]
    return _original_reshape(self, *static_shape)
torch.Tensor.reshape = patched_reshape

# 4. Fix OTO Group Division issue
...
# ==========================================
# 5. EVALUATION FUNCTION (mAP) - 🎯 CORRECTED
# ==========================================
        ...
        for img_id in tqdm(val_ids, desc="Evaluating", leave=False):
            ...
            for s, idx in zip(scores, indexes):
                if s > 0.05:
                    ...
                    # 🎯 Core Modification: Take the category number directly without an intermediate matrix
                    cat_id = int(c_idx)
                    
                    # Ignore the zero category (since COCO starts from 1)
                    if cat_id == 0:
                        continue
                    ...
                    results.append({
                        "image_id": img_id, 
                        "category_id": cat_id,
                        "bbox": [x_min, y_min, w * w_orig, h * h_orig], 
                        "score": float(s)
                    })
